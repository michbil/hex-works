function stringToByteSeq(buffer) {
    ...
    if ((data.length % 2) !== 0) { //add extra zero to output
        data = '0' + data;
    }
...
function toHex(number, length) {
    ...
    if (typeof number === "undefined") {
        console.log("Assert, null trying to be displayed");
        return "ZZ";
    }
...
function hexInvert(s) {
    ...
    var data = s.replace(/[^0-9a-fA-F]/g,"")
    var bytelist = data.match(/.{1,2}/g);
    var res = "";

    for (var i in bytelist) {
        var val = parseInt(bytelist[i],16);
        val = val ^ 0xFF;
...
function reverseByteString(s) {
    ...
    for (var i=0;i< s.length;i+=2) {
        res = res + s[s.length-2-i];
        res = res + s[s.length-1-i];
    }
    return res;
}

...
export function useHexNavigation() {
  ...
  const moveCursor = useCallback(
    (direction: 'left' | 'right' | 'up' | 'down', extend: boolean = false) => {
      if (!buffer) return;
    ...
    [buffer, cursorPosition, bytesPerLine, bufferLength, setCursorPosition, setSelection]
  );
  ...
  const goToEnd = useCallback(
    ...
    [bufferLength, setCursorPosition, setSelection]
  );
  ...
  const pageUp = useCallback(() => {
    ...
    setCursorPosition(newPosition);
    setSelection(newPosition, newPosition);
  }, [cursorPosition, bytesPerLine, setCursorPosition, setSelection]);
  ...
  const pageDown = useCallback(() => {
    ...
    setCursorPosition(newPosition);
    setSelection(newPosition, newPosition);
  }, [cursorPosition, bytesPerLine, bufferLength, setCursorPosition, setSelection]);
